...
signed main(){
	...
	while(ab.size()>1){
		ans.pub(mkp(ab.front(),ab.back()));
		ab.pof();
		ab.pob();
	}
	while(ba.size()>1){
		ans.pub(mkp(ba.front(),ba.back()));
		ba.pof();
		ba.pob();
	}
	if(ab.size()==1&&ba.size()==1)
		ans.pub(mkp(ab[0],ab[0])),
		ans.pub(mkp(ab[0],ba[0]));
	cout<<ans.size()<<endl;
	fo(i,0,ans.size()) cout<<ans[i].fst<<' '<<ans[i].snd<<endl;
    return 0;
}
