...
string s;
int dp[2][455][455];
//in i position,changed j times,now have been working k minutes
...
signed main()
{
	...
	re(i,1,n)
    {
		re(j,0,n) re(k,0,n) dp[now][j][k] = inf;
		re(j,0,i)
			re(k,0,i)
			{
				if(s[i]=='1')
				{
					if(k!=0)
						dp[now][j][k]=min(dp[now][j][k],dp[pre][j][k-1]+k);
					if(j!=0)
						dp[now][j][0]=min(dp[now][j][0],dp[pre][j-1][k]);
				}
				else if(s[i]=='0')
					dp[now][j][0]=min(dp[now][j][0],dp[pre][j][k]);
			}
		if(i!=n) swap(now,pre);
	}
//	re(j,0,n) re(k,0,n) cout<<dp[now][j][k]<<" \n"[k==n];
	int ans=inf;
	re(j,0,n)
		re(k,0,n)
			if(dp[now][j][k]<=m)
