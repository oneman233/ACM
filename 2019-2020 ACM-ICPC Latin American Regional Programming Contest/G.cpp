...
{
	if(cmp(a.x,b.x)==0) return a.bri<b.bri;
...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		scanf("%lf%lf%d",&p[i].x,&p[i].y,&p[i].bri);
		//p[i].x-=1e4;
		//p[i].y-=1e4;
	}
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<n;j++)
			t[j]=proj(st,p[i],p[j]),t[j].bri=p[j].bri;
		sort(t,t+n,cmp1);
		int a=0,b=0;
		for(int j=0;j<n-1;j++)
			if(t[j].bri>t[j+1].bri) a++;
			else if(t[j].bri<t[j+1].bri) b++;
		if(a==0||b==0) return printf("Y %d",i),0;
		sort(t,t+n,cmp2);
		a=0,b=0;
		for(int j=0;j<n-1;j++)
			if(t[j].bri>t[j+1].bri) a++;
