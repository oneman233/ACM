...
signed main(){
    ...
    cin>>n>>m;
    re(i,1,n) re(j,1,m) cin>>c[i][j];
    re(i,1,n) re(j,1,m) a[i][j]=1-(c[i][j]-'0');
    re(i,1,n) re(j,1,m) s[i][j]=s[i-1][j]+s[i][j-1]-s[i-1][j-1]+a[i][j];
    int ans=0;
    re(i,1,n){
		re(j,1,m){
			re(p,1,n){
				re(q,1,m){
					if(cal(i,j,p,q)==(abs(i-p)+1)*(abs(j-q)+1)){
						ans=max(ans,2*((abs(i-p)+1)+(abs(j-q)+1)));
					}
				}
			}
		}
