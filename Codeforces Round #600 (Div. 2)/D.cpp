...
void _merge(int x,int y){
	x=_find(x),y=_find(y);
	if(x!=y) f[x]=y,mx[y]=max(mx[y],mx[x]),mn[y]=min(mn[x],mn[y]);
}

signed main(){
	FAST
    cin>>n>>m;
    re(i,1,n) f[i]=i,mx[i]=i,mn[i]=i;
    re(i,1,m){
		cin>>x[i]>>y[i];
		_merge(x[i],y[i]);
	}
	re(i,1,n){
		if(!vis[_find(i)]){
			vis[_find(i)]=1;
			s.insert(mkp(mn[_find(i)],mx[_find(i)]));
		}
	}
	each(i,s){
		auto j=i;
		++i;
		if(i==s.end()) break;
		int l1=(*j).fst,r1=(*j).snd;
		int l2=(*i).fst,r2=(*i).snd;
		if((l1<=l2&&l2<=r1)||(l1<=r2&&r2<=r1)){
			ans++;
			s.erase(i);
			s.erase(j);
			s.insert(mkp(min(l1,l2),max(r1,r2)));
		}
	}
	cout<<ans;
	return 0;
