...
signed main()
{
    ...
    cin>>n>>m>>b;
    swap(n,m);
    re(i,1,n) re(j,1,m) cin>>mp1[i][j];
    while(b--)
    {
		re(i,1,n) re(j,1,m) mp2[i][j]=cal(i,j);
		re(i,1,n) re(j,1,m) mp1[i][j]=mp2[i][j];
	}
//	re(i,1,n) re(j,1,m) cout<<mp1[i][j]<<" \n"[j==m];
	re(i,1,n) re(j,1,m) s.insert(mp1[i][j]);
	cout<<s.size();
    return 0;
}
