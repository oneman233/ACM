...
int n;
vei e[maxn];
int sz[maxn],sum[maxn],sum2[maxn];
...
void dfs1(int x,int f)
{
	...
	for(auto y:e[x])
	{
		...
		dfs1(y,x);
		sz[x]=(sz[x]+sz[y])%mod;
		sum[x]=(sum[x]+sum[y]+sz[y])%mod;
		sum2[x]=(sum2[x]+sum2[y]+2*sum[y]%mod+sz[y])%mod;
	}
}

int cal(int x,int f)
{
	return (ans[f]-sum[x]-sz[x]+mod)%mod;
}

int cal2(int x,int f)
{
	return (ans2[f]-sum2[x]-2*sum[x]%mod-sz[x]+mod)%mod;
}
...
void dfs2(int x,int f)
{
	if(x!=1)
	{
		ans[x]=((cal(x,f)+sum[x])%mod+(n-sz[x])+mod)%mod;
		ans2[x]=((cal2(x,f)+2*cal(x,f)%mod)%mod+(n-sz[x])+mod+sum2[x])%mod;
	}
	for(auto y:e[x])
	{
		if(y==f) continue;
...
signed main()
{
	...
	ans2[1]=sum2[1];
	dfs2(1,1);
//	re(i,1,n) cout<<ans[i]<<' ';cout<<endl;
//	re(i,1,n) cout<<ans2[i]<<' ';cout<<endl;
	int tmp=0;
	re(i,1,n) tmp=(tmp+ans2[i])%mod;
	cout<<tmp;
    return 0;
}
/*
