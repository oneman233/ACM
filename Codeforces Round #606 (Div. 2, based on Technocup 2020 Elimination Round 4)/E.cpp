...
int t,n,m,a,b,sz[maxn];
vei e[maxn],part[maxn];
bool is[maxn];
...
{
	re(i,1,n)
		e[i].clear(),part[i].clear();
	re(i,1,n)	
		is[i]=0,dfn[i]=low[i]=sz[i]=0;
	timer=0;
	re(i,1,n) visa[i]=visb[i]=0;
}
...
//			nmsl;
			for(auto to:e[a])
	        	if(!visa[to]&&to!=b) dfsa(to,a);
	    	for(auto to:e[b])
	        	if(!visb[to]&&to!=a) dfsb(to,a);
			int A=0,B=0;
			for(int i=1;i<=n;i++)
			{
			    if(i==a||i==b) continue;
