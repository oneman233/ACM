...
void pre(){
	ex[0]=1;
	re(i,1,100000) ex[i]=(ex[i-1]+ex[i-1])%MOD;
}
...
void gao(){
	...
	re(i,1,q){
		...
		if(lb.have(o[i].x)) ans[o[i].id]=ex[o[i].l-lb.sz];
		else ans[o[i].id]=0;
	}
}

signed main(){
    cin>>n>>q;re(i,1,n) cin>>a[i];
    re(i,1,q) cin>>o[i].l>>o[i].x,o[i].id=i;
    sort(o+1,o+1+q,cmp);
    pre();
    gao();
