...
void chk(int i,int j)
{
	fo(k,0,8)
	{
		int ii=i+mvi[k],jj=j+mvj[k];
		int iii=ii+mvi[k],jjj=jj+mvj[k];
		if(ok(ii,jj)&&ok(iii,jjj))
		{
			if(mp[ii][jj]=='i'&&mp[iii][jjj]=='e') ans=1;
		}
	}
}
