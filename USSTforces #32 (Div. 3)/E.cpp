...
int n;
db t1,t2,k;
db v[maxn],u[maxn],ans[maxn];
pdi p[maxn];

bool cmp(pdi a,pdi b){
	if(!eql(a.fst,b.fst)) return a.fst>b.fst;
...
signed main(){
	FAST
	cin>>n>>t1>>t2>>k;
	re(i,1,n) cin>>v[i]>>u[i];
	cout<<fixed<<setprecision(2);
	re(i,1,n){
		ans[i]=v[i]*t1*(100.0-k)/100.0+u[i]*t2;
		ans[i]=max(ans[i],u[i]*t1*(100.0-k)/100.0+v[i]*t2);
	}
//	re(i,1,n) cout<<i<<' '<<ans[i]<<endl;
	re(i,1,n) p[i].fst=ans[i],p[i].snd=i;
	sort(p+1,p+1+n,cmp);
	re(i,1,n) cout<<p[i].snd<<' '<<p[i].fst<<endl;
	return 0;
}
