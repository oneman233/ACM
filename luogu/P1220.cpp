...
signed main()
{
    FAST
    cin>>n>>s;
    re(i,1,n) cin>>a[i]>>b[i],sum[i]=sum[i-1]+b[i];
    memmx(dp);
    dp[s][s][0]=dp[s][s][1]=0;
    re(l,2,n)
    {
		re(i,1,n)
		{
			int j=i+l-1;
			if(j>n) break;
			dp[i][j][0]=mmin(dp[i][j][0],
			dp[i+1][j][0]+(a[i+1]-a[i])*(ss(1,i)+ss(j+1,n)),
			dp[i+1][j][1]+(a[j]-a[i])*(ss(1,i)+ss(j+1,n)));
			dp[i][j][1]=mmin(dp[i][j][1],
			dp[i][j-1][1]+(a[j]-a[j-1])*(ss(1,i-1)+ss(j,n)),
			dp[i][j-1][0]+(a[j]-a[i])*(ss(1,i-1)+ss(j,n)));
		}
	}
