...
void dfs2(int x)
{
	...
	vis[x]=1;
//	cout<<"$ "<<x<<endl;
	dp2[x]=max(dp2[x],(int)scc[x].size());
	for(auto i:e2[x])
		dp2[i]=max(dp2[i],dp2[x]+(int)scc[i].size()),
		dfs2(i);
}
...
void dfs3(int x)
{
	if(vis[x]) return;
	vis[x]=1;
	dp3[x]=max(dp3[x],(int)scc[x].size());
	for(auto i:e3[x])
		dp3[i]=max(dp3[i],dp3[x]+(int)scc[i].size()),
		dfs3(i);
}

void gao()
{
//	re(i,1,n) cout<<"! "<<i<<' '<<co[i]<<endl;
	re(i,1,m)
		if(co[x[i]]!=co[y[i]])
			e2[co[x[i]]].pub(co[y[i]]),
			e3[co[y[i]]].pub(co[x[i]]);
	memmn(dp2),memmn(dp3);
	re(i,1,color) vis[i]=0;
	dfs2(co[1]);
	re(i,1,color) vis[i]=0;
	dfs3(co[1]);
//	re(i,1,color) cout<<"? "<<i<<" dp2 "<<dp2[i]<<" dp3 "<<dp3[i]<<endl;
	re(i,1,m)
	{
		if(co[x[i]]==co[y[i]]) continue;
		int xx=co[x[i]];
		int yy=co[y[i]];
		ans=max(ans,dp3[xx]+dp2[yy]-(int)scc[co[1]].size());
		ans=max(ans,dp2[xx]+dp3[yy]-(int)scc[co[1]].size());
	}
	cout<<ans;
}
