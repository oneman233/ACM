...
struct graph
{
	...
	db rate;
	graph(){}
	graph(int Y,int V,db Rate):y(Y),v(V),rate(Rate){}
};
vector<graph> g1[maxn];
vep g2[maxn];
struct edge
{
	int x,y,v;
	db rate;
	edge(){}
	edge(int X,int Y,int V,db Rate):x(X),y(Y),v(V),rate(Rate){}
}e[maxn];
...
void topo()
{
	...
	while(!q.empty())
	{
		...
		for(auto i:g2[f])
		{
			dp[i.fst]=max(dp[i.fst],dp[f]+i.snd+sz[i.fst]);
			deg[i.fst]--;
			if(!deg[i.fst])
				q.push(i.fst);
		}
...
//	freopen("P2656_7.in","r",stdin);
    ...
    re(i,1,m)
    {
		cin>>e[i].x>>e[i].y>>e[i].v>>e[i].rate;
		g1[e[i].x].pub(graph(e[i].y,e[i].v,e[i].rate));
	}
	cin>>S;
	re(i,1,n)
		if(!dfn[i])
	...
	re(i,1,m)
	{
		...
		{
			g2[co[e[i].x]].pub(mkp(co[e[i].y],e[i].v));
			deg[co[e[i].y]]++;
		}
	}
	topo();
	int ans=0;
	re(i,1,color) ans=max(ans,dp[i]);
	cout<<ans;
    return 0;
}
