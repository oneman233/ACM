...
int n,m;
BigNum a[85],dp[85][85],ans=0;

BigNum dfs(int l,int r,int cnt)
{
	if(dp[l][r]!=-1) return dp[l][r];
	BigNum add=1;
	re(i,1,cnt) add=add*2;
	if(l==r) return a[l]*add;
	dp[l][r]=max(a[l]*add+dfs(l+1,r,cnt+1),a[r]*add+dfs(l,r-1,cnt+1));
	return dp[l][r];
}
