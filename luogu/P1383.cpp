...
int modi(int rt,int l,int r,int pos,char ch)
{
	int p=++tot;
	tr[p]=tr[rt];
	if(l==r)
	...
	int m=mm(l,r);
	if(pos<=m) tr[p].ls=modi(tr[rt].ls,l,m,pos,ch);
	else tr[p].rs=modi(tr[rt].rs,m+1,r,pos,ch);
}

char ask(int rt,int l,int r,int pos)
{
	if(l==r) return tr[rt].ch;
	int m=mm(l,r);
	if(pos<=m) return ask(tr[rt].ls,l,m,pos);
	else return ask(tr[rt].rs,m+1,r,pos);
}
