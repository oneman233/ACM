...
int n,a[1005];
BigNum dp1[1005],dp0[1005],change[1005];

int main(){
	scanf("%d",&n);
	for(int i=1;i<=n;++i)
		scanf("%d",&a[i]);
	dp1[1]=dp0[1]=change[1]=1;
	for(int i=2;i<=n;++i){
		change[i]=2*change[i-1]+1;
		if(a[i]==1){
			dp1[i]=min(dp0[i-1],dp1[i-1]+change[i-1]);
			dp0[i]=min(dp1[i-1]+1+change[i-1],dp0[i-1]+change[i-1]+1+change[i-1]);
		}
		else{
			dp1[i]=min(dp1[i-1]+1+change[i-1],dp0[i-1]+change[i-1]+1+change[i-1]);
			dp0[i]=min(dp0[i-1],dp1[i-1]+change[i-1]);
		} 
	}
	dp0[n].print();
	return 0;
