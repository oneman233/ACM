...
//consider the difference between both players
...
signed main()
{
    ...
    re(i,1,n) re(j,1,m) dp[i][j][a[i][j]][0]=1;
    int ans=0;
    re(i,1,n)
    {
		re(j,1,m)
		{
			fo(k,0,x)
			{
				dp[i][j][(k+a[i][j])%x][0]+=dp[i-1][j][k][1];
				dp[i][j][(k+a[i][j])%x][0]+=dp[i][j-1][k][1];
				dp[i][j][(k-a[i][j]+x)%x][1]+=dp[i-1][j][k][0];
				dp[i][j][(k-a[i][j]+x)%x][1]+=dp[i][j-1][k][0];
				//
				dp[i][j][(k+a[i][j])%x][0]%=mod;
				dp[i][j][(k-a[i][j]+x)%x][1]%=mod;
			}
		}
	}
	re(i,1,n) re(j,1,m) ans=(ans+dp[i][j][0][1])%mod;
	cout<<ans;
    return 0;
}
/*
