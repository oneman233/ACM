...
int n,m;
int a[maxn],b[maxn],sum[maxn],ans[maxn];
int pre[maxn];
int s(int l,int r){
...
signed main(){
    ...
    n=read();
    re(i,1,n) a[i]=read(),b[i]=a[i],sum[i]=sum[i-1]^a[i];
    sort(b+1,b+1+n);
    re(i,1,n) a[i]=lower_bound(b+1,b+1+n,a[i])-b;
    m=read();
    re(i,1,m) q[i].l=read(),q[i].r=read(),q[i].id=i;
    sort(q+1,q+1+m,cmp);
	int f=q[1].l;
    re(i,1,m){
		int L=q[i].l,R=q[i].r;
		...
		ans[q[i].id]=ask(R)^ask(L-1)^s(L,R);
	}
    re(i,1,m) write(ans[i]),prn();
    return 0;
}
