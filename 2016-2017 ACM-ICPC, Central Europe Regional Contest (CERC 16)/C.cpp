...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		if(s[i]=='T') 
		{
			pol.emplace_back(i,0);
			pol.emplace_back(i+1,0);
			pol.emplace_back(i+0.5,sqrt(3)/2);
		}
		else if(s[i]=='S')
		{
			pol.emplace_back(i,0);
			pol.emplace_back(i,1);
			pol.emplace_back(i+1,0);
			pol.emplace_back(i+1,1);
		}
		else 
		{
			P t=P(0,0.5),k;
			...
			for(int j=0;j<N;j++)
			{
				pol.emplace_back(i+0.5+k.x,0.5+k.y);
				k=t.rot(ang*j);
			}
		}
	}
	...
	for(int i=0;i<pol.size()-1;i++)
		ans+=pol[i].distTo(pol[i+1]);
	ans+=pol[0].distTo(pol[pol.size()-1]);
	printf("%.12f",ans);
	return 0;	
}
