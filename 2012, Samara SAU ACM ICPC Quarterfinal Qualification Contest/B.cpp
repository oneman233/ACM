...
int main()
{
	...
	int ax=0,ay=0,r=0;
	double ans=1e18,A=area(con),now=0;
	for(int l=0;l<n;l++)
	{
		while(1)
		{
			int nxt=(r+1)%n;
			double da=((con[r]-con[l]).det(con[nxt]-con[l]))/2;
			if(da<0) da=-da;
			if(now+da<=A-now-da+EPS) now+=da,r++,r%=n; else break;
		}
		if(A-now-now<ans) ans=A-now-now,ax=l,ay=r;
		int nxt=(l+1)%n;
		double da=((con[l]-con[r]).det(con[nxt]-con[r]))/2;
		now-=da;
	}
