...
int n,m,k,ans=0ll;
int add[505][505];
int hang[505][2],lie[505][2];

int sum(char c,int p,int t,int tag) {
	int ret=0ll;
	if(c=='r') {
		re(j,1,m) {
			int tmp=-1;
			if(hang[p][1]<lie[j][1]) tmp=hang[p][0];
			else tmp=lie[j][0];
			tmp=t-tmp;
			tmp%=mod;
			ret=(ret+tmp*add[p][j]%mod)%mod;
		}
		hang[p][0]=t;
		hang[p][1]=tag;
	}
		...
		re(i,1,n) {
			int tmp=-1;
			if(hang[i][1]<lie[p][1]) tmp=hang[i][0];
			else tmp=lie[p][0];
			tmp=t-tmp;
			tmp%=mod;
			ret=(ret+tmp*add[i][p]%mod)%mod;
		}
		lie[p][0]=t;
		lie[p][1]=tag;
	}
...
signed main() {
    ...
    re(i,1,n) re(j,1,m) add[i][j]=read(),add[i][j]%=mod,hang[i][1]=k+1,lie[j][1]=k+1;
	while(k--) {
    	char c;
    	int p,t;
		scanf("%c%lld%lld",&c,&p,&t);
		getchar();
		ans=(ans+sum(c,p,t,k))%mod;
	}
	write(ans);
    return 0;
}
