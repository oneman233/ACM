...
void gao(int now) {
	int tmp=gen();
//	cout<<"b:"<<tmp<<endl;
	while(!s.empty()&&tmp<s.top().fst) sum.pop(),s.pop();
	int k=-1;
	if(sum.empty()) ret=(ret+tmp*now%mod)%mod,k=tmp*now%mod;
	else ret=(ret+(sum.top()%mod+tmp*(now-s.top().snd)%mod)%mod)%mod,
	k=sum.top()%mod+tmp*(now-s.top().snd)%mod;
//	cout<<"ret:"<<ret<<endl;
	s.push({tmp,now});
	sum.push(k);
	a=ret;
