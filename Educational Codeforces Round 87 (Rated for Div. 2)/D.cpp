...
void add(int pos,int x){
	for(int i=pos;i<=1000000;i+=lowb(i))
		tr[i]+=x;
	cnt[pos]+=x;
}
...
signed main()
{
    FAST
    n=read(),q=read();
    re(i,1,n) a=read(),add(a,1);
    while(q--){
		k=read();
		if(k<0) out(-k);
		else add(k,1);
	}
//	re(i,1,1000000) while(cnt[i]) {cout<<i<<' ';cnt[i]--;}
	re(i,1,1000000) if(cnt[i]!=0) {write(i);return 0;}
	write(0);
    return 0;
}
/*
