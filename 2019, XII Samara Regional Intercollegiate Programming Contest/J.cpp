...
signed main()
{
    ...
    re(i,1,n) sum[i]=a[i]+b[i]+c[i];
    re(i,1,n) mn[i]=mmin(a[i]+b[i]+2,a[i]+c[i]+2,b[i]+c[i]+2),mem[i]=mn[i];
    sort(mn+1,mn+1+n);
	re(i,1,n)
    {
		int ans=upper_bound(mn+1,mn+1+n,sum[i])-mn-1;
		if(sum[i]>=mem[i]) ans--;
		cout<<ans<<' ';
	}
	return 0;
