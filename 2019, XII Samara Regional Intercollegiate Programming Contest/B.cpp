...
signed main()
{
    FAST
    re(i,1,2) cin>>s[i];
    int up=0,down=0,both=0,res=0;
    fo(i,0,s[1].length())
    {
		if(s[1][i]=='#'&&s[2][i]=='.') up++;
		else if(s[1][i]=='.'&&s[2][i]=='#') down++;
		else if(s[1][i]=='#'&&s[2][i]=='#') both++;
		else res++;
	}
	...
	cout<<"YES"<<endl;
	while(up--) ans[1].pub('#'),ans[2].pub('.');
	while(both--) ans[1].pub('#'),ans[2].pub('#');
	while(down--) ans[1].pub('.'),ans[2].pub('#');
	while(res--) ans[1].pub('.'),ans[2].pub('.');
	fo(i,0,ans[1].size()) cout<<ans[1][i];
	cout<<endl;
	fo(i,0,ans[2].size()) cout<<ans[2][i];
    return 0;
}
