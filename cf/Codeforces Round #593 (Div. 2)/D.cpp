...
bool gao(int d){
	sum=n*m-k-1;
	int nowx=1,nowy=1,dir=d;
	int rowL=0,rowR=n+1,colL=0,colR=m+1;
	re(i,1,n+m){
		int tmp,lastx=nowx,lasty=nowy;
		if(dir==1){
			tmp=min(colR,*row[nowx].lower_bound(nowy));
			sum-=tmp-nowy-1;
			nowy=tmp-1;
			colR=nowy;
			rowL=nowx;
			...
			tmp=min(rowR,*col[nowy].lower_bound(nowx));
			sum-=tmp-nowx-1;
			nowx=tmp-1;
			colR=nowy;
			rowR=nowx;
			...
			auto ii=row[nowx].lower_bound(nowy);
			ii--;
			tmp=max(colL,*ii);
			sum-=nowy-tmp-1;
			nowy=tmp+1;
			...
			auto ii=col[nowy].lower_bound(nowx);
			ii--;
			tmp=max(rowL,*ii);
			sum-=nowx-tmp-1;
			nowx=tmp+1;
...
signed main(){
	...
	re(i,1,n) row[i].insert(0),row[i].insert(m+1);
	re(i,1,m) col[i].insert(0),col[i].insert(n+1);
	if(gao(1)||gao(2)) cout<<"Yes";
	else cout<<"No";
    return 0;
}
