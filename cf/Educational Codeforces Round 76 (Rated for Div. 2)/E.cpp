...
void gao(){
	sort(a[1]+1,a[1]+1+k[1]);
	sort(a[2]+1,a[2]+1+k[2]);
	sort(a[3]+1,a[3]+1+k[3]);
	v.pub(-1);
	re(j,1,3) re(i,1,k[j]) v.pub(a[j][i]);
//	re(i,1,k[1]+k[2]+k[3]) cout<<v[i]<<' ';cout<<endl;
	dp[1]=v[1];
	re(i,2,k[1]+k[2]+k[3]){
		if(v[i]>dp[lst]) dp[++lst]=v[i];
		else{
			int p=lower_bound(dp+1,dp+1+lst,v[i])-dp;
			dp[p]=v[i];
		}
	}
	cout<<k[1]+k[2]+k[3]-lst;
}

signed main(){
	FAST
    cin>>k[1]>>k[2]>>k[3];
    re(j,1,3) re(i,1,k[j]) cin>>a[j][i];
    gao();
	return 0;
}
