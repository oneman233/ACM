...
void add(int L,int R,int v,int p=1,int l=1,int r=2e5){
	if(L<=l&&r<=R){
		mx[p]+=v;
		tag[p]+=v;
		return;
	...
	int m=mm(l,r);
	down(p);
	if(L<=m) add(L,R,ll(p),l,m);
	if(R>m) add(L,R,rr(p),m+1,r);
	up(p);
}

int ask(int L,int R,int p=1,int l=1,int r=2e5){
	if(L<=l&&r<=R) return mx[p];
	int m=mm(l,r),ret=-inf;
	down(p);
	if(L<=m) ret=max(ret,ask(L,R,ll(p),l,m));
	if(R>m) ret=max(ret,ask(L,R,rr(p),m+1,r));
	up(p);
	return ret;
}
